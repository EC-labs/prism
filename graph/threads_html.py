...
tid_futex = conn.execute("""
    ...
    SELECT DISTINCT 
        pid, tid, op, futex_key_addr, futex_key_word, futex_key_offset 
    FROM (
        SELECT * FROM fwait
        UNION ALL
        SELECT * FROM fwake
    ...
    ORDER BY 
        futex_key_addr, futex_key_word, futex_key_offset, pid, tid, op
""").df()
...
def build_graph_with_clusters(tid_futex: pd.DataFrame) -> nx.Graph:
    ...
    for _, row in tid_futex.iterrows():
        op = row["op"]
        tid = row["tid"]
        ...
        # Add undirected edge
        if op == "wake":
            G.add_edge(tid, futex_key, color='rgba(150,150,150,0.6)')#, label=f"{num_connections}")
        else:
            G.add_edge(futex_key, tid, color='rgba(150,150,150,0.6)')#, label=f"{num_connections}")
