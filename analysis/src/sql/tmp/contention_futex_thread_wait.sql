...
INNER JOIN contention as c
    ON fw_.futex_key_addr = c.futex_key_addr
    AND fw_.futex_key_word = c.futex_key_word
    AND fw_.futex_key_offset = c.futex_key_offset
WHERE 
    fw_.tid in (SELECT DISTINCT tid from fwait ORDER BY tid LIMIT 1 OFFSET 1)
ORDER BY fw_.ts_s
