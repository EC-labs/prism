...
// use futex::types::{granularity, stats};
use futex::*;
...
impl<'obj, 'conn> Futex<'obj, 'conn> {
    pub fn new(
        ...
        Ok(Self {
            ...
            // appender: conn.appender("futex")?,
            // staging_appender: conn.appender("futex_staging")?,
            // conn,
            // updated: HashMap::new(),
        })
    }
    ...
    //         INSERT INTO futex (ts_s, pid, tid, fs_id, device_id, inode_id, is_write, total_time)
    //             SELECT
    //                 vs.*
    //             FROM futex_staging as vs
    //             LEFT JOIN futex as v
    //                 USING (ts_s, pid, tid, fs_id, device_id, inode_id, is_write)
    //             WHERE
    //                 v.ts_s IS NULL;

    //         DELETE FROM futex_staging;
    //         ",
    //     )?;
    //     Ok(())
    // }
